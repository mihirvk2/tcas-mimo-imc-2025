...
def sigma_adc_sweep(channel, modulation, nsamples):
    N, K = tuple(map(int, re.findall(r'\d+', channel)[-2:]))
    H = extract_H(channel)
    B_y = 6
    B_w = 6
    B_ADC = int(np.ceil(np.log2(N)))-1
    sigma_ADC_min = 0.0001
    sigma_ADC_max = 0.0016
    npoints = 16
    sigma_ADC_options = np.linspace(sigma_ADC_min,sigma_ADC_max,npoints)
    SNRdB = 20 
    cap = 1
    VDD = 0.9
    ...
    logging.basicConfig(filename=f'Outputs/adc_noise_sweep_{channel}_{modulation}.log', level=logging.INFO, filemode='w')
    logging.info("EVM LMMSE = {}".format(evm_lmmse_fp))
    logging.info("EVM LMMSE FX = {}".format(evm_lmmse_fx))

    mimo_imc_lmmse = mimo_imc(n_phy_rows=int(2**(np.ceil(np.log2(N)))), n_phy_cols=K*B_w, b_w = B_w, b_x=B_y, b_adc= B_ADC,c_qr_mean=cap, v_dd= VDD, sigma_adc=0.0005)
    mimo_imc_lmmse.store_complex_weights(A_H_LMMSE, A_H_LMMSE_ranges)
    mimo_imc_lmmse.update_b_adc_mimo(N*0.25, B_ADC)
    for idx in range(npoints):
        print(f"Starting simulation for ADC noise = {sigma_ADC_options[idx]}")
        for i in tqdm(range(nsamples)):
            mimo_imc_lmmse.update_sigma_adc(sigma_ADC_options[idx])
            xt[:,i] = generate_input_vector(K, modulation)[0]
            # AWGN with specified SNR
            awgn_2d = np.random.multivariate_normal(np.zeros(2),np.eye(2)*N0/2,size=N) 
            n = awgn_2d[:,0] + awgn_2d[:,1]*complex(0,1) 
        ...
        logging.info("Finished simulation for ADC noise = {}".format(sigma_ADC_options[idx]))
        logging.info("EVM LMMSE IMC = {} \n".format(evm_lmmse_IMC[idx]))

    np.save(f'Outputs/sigma_adc_options_{channel}_{modulation}.npy', sigma_ADC_options)
    np.save(f'Outputs/evm_fp_sigma_adc_{channel}_{modulation}.npy', evm_lmmse_fp)
    np.save(f'Outputs/evm_fx_sigma_adc_{channel}_{modulation}.npy', evm_lmmse_fx)
    np.save(f'Outputs/evm_imc_sigma_adc_{channel}_{modulation}.npy', evm_lmmse_IMC)
    
    plt.figure(figsize=(6,4))
    plt.plot(sigma_ADC_options*1000, 20*np.log10(evm_lmmse_IMC), '-^', label='IMC', markerfacecolor='none', markeredgecolor='red', color = 'red')
    ...
    plt.grid()
    plt.xlabel(r'$\sigma_{\mathrm{ADC}}$ (mV)',fontsize = 13)
    plt.ylabel('EVM (dB)', fontsize = 13)
    plt.legend(fontsize=8)
    plt.savefig(f'Figures/sigma_adc_sweep_{channel}_{modulation}.png', format='png', bbox_inches='tight')

def plot_evm_sigma_adc_all():

    evm_WINNER_64_8_64QAM_IMC = np.load('Outputs/evm_imc_sigma_adc_WINNER_64_8_64QAM.npy')
    evm_WINNER_64_8_64QAM_FX = np.load('Outputs/evm_fx_sigma_adc_WINNER_64_8_64QAM.npy')
    evm_WINNER_128_16_64QAM_IMC = np.load('Outputs/evm_imc_sigma_adc_WINNER_128_16_64QAM.npy')
    evm_WINNER_128_16_64QAM_FX = np.load('Outputs/evm_fx_sigma_adc_WINNER_128_16_64QAM.npy')
    evm_WINNER_256_16_64QAM_IMC = np.load('Outputs/evm_imc_sigma_adc_WINNER_256_16_64QAM.npy')
    evm_WINNER_256_16_64QAM_FX = np.load('Outputs/evm_fx_sigma_adc_WINNER_256_16_64QAM.npy')
    evm_ARGOS_96_8_64QAM_IMC = np.load('Outputs/evm_imc_sigma_adc_ARGOS_96_8_64QAM.npy')
    evm_ARGOS_96_8_64QAM_FX = np.load('Outputs/evm_fx_sigma_adc_ARGOS_96_8_64QAM.npy')

    sigma_ADC_options = np.linspace(0.1,1.6,16)

    plt.figure(figsize=(6,4))
    plt.plot(sigma_ADC_options, 20*np.log10(evm_WINNER_64_8_64QAM_IMC), '-^', label=r'WIN-64x8, 64-QAM', markerfacecolor='none', markeredgecolor='red', color = 'red')
    plt.axhline(y = 20*np.log10(evm_WINNER_64_8_64QAM_FX), linestyle = 'dashed', color = 'red', linewidth = 1)
    plt.plot(sigma_ADC_options, 20*np.log10(evm_WINNER_128_16_64QAM_IMC), '-^', label=r'WIN-128x16, 64-QAM', markerfacecolor='none', markeredgecolor='green', color = 'green')
    plt.axhline(y = 20*np.log10(evm_WINNER_128_16_64QAM_FX), linestyle = 'dashed', color = 'green', linewidth = 1.2)
    plt.plot(sigma_ADC_options, 20*np.log10(evm_WINNER_256_16_64QAM_IMC), '-^', label=r'WIN-256x16, 64-QAM', markerfacecolor='none', markeredgecolor='blue', color = 'blue')
    plt.axhline(y = 20*np.log10(evm_WINNER_256_16_64QAM_FX), linestyle = 'dashed', color = 'blue', linewidth = 1, alpha = 0.6)
    plt.plot(sigma_ADC_options, 20*np.log10(evm_ARGOS_96_8_64QAM_IMC), '-^', label=r'ARGOS-96x8, 64-QAM', markerfacecolor='none', markeredgecolor='orange', color = 'orange')
    ...
    plt.ylabel('EVM (dB)',fontsize = 13)
    # plt.legend(fontsize=8)
    plt.savefig(f'Figures/evm_sigma_adc_sweep_all.png', format='png', bbox_inches='tight')
    # plt.show()


if __name__ == '__main__':

    # sigma_adc_sweep('WINNER_64_8', 'QPSK', 20000)
    # sigma_adc_sweep('WINNER_128_16', 'QPSK', 10000)
    # sigma_adc_sweep('WINNER_256_16', 'QPSK', 10000)
    # sigma_adc_sweep('ARGOS_96_8', 'QPSK', 20000)
    # sigma_adc_sweep('WINNER_64_8', '16QAM', 20000)
    # sigma_adc_sweep('WINNER_128_16', '16QAM', 10000)
    # sigma_adc_sweep('WINNER_256_16', '16QAM', 10000)
    # sigma_adc_sweep('ARGOS_96_8', '16QAM', 20000)
    # sigma_adc_sweep('WINNER_64_8', '64QAM', 6000)
    # sigma_adc_sweep('WINNER_128_16', '64QAM', 2500)
    # sigma_adc_sweep('WINNER_256_16', '64QAM', 3000)
    # sigma_adc_sweep('ARGOS_96_8', '64QAM', 5000)

    plot_evm_sigma_adc_all()